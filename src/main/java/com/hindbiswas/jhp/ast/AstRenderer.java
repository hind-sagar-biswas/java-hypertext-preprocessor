...
public class AstRenderer {
    ...
    private static class ControlFlow extends RuntimeException {
        ...
        ControlFlow(String tag) { this.tag = tag; }
    }
    ...
    private void renderElements(List<TemplateElementNode> elements, Deque<Map<String,Object>> scopes, StringBuilder sb) {
    ...
    private void renderElement(TemplateElementNode el, Deque<Map<String,Object>> scopes, StringBuilder sb) {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (fn.updateOp != null) {
                        ...
                        if (n == null) { local.put(fn.updateIdentifier, 1L); }
                        else {
                            if ("++".equals(fn.updateOp)) local.put(fn.updateIdentifier, n.longValue() + 1);
                            else local.put(fn.updateIdentifier, n.longValue() - 1);
                        }
        ...
        if (el instanceof ForeachNode) {
            ...
            if (it instanceof Map) {
                ...
                for (Map.Entry<?,?> e : m.entrySet()) {
                    Map<String,Object> local = new HashMap<>();
                    if (fe.keyIdentifier != null) local.put(fe.keyIdentifier, e.getKey());
                    local.put(fe.valueIdentifier, e.getValue());
                    scopes.push(local);
                    try {
                        renderElements(fe.body, scopes, sb);
                    } catch (ControlFlow cf) {
                        if ("break".equals(cf.tag)) { scopes.pop(); break; }
                        // continue -> just pop and continue
                    } finally {
                        if (!scopes.isEmpty() && scopes.peek() == local) scopes.pop();
                    }
                }
                ...
                for (Object item : itr) {
                    ...
                    if (fe.keyIdentifier != null) local.put(fe.keyIdentifier, null);
                    scopes.push(local);
                    try {
                        renderElements(fe.body, scopes, sb);
                    } catch (ControlFlow cf) {
                ...
                for (int i=0;i<len;i++) {
                    ...
                    Map<String,Object> local = new HashMap<>();
                    if (fe.keyIdentifier != null) local.put(fe.keyIdentifier, i);
                    local.put(fe.valueIdentifier, item);
                    scopes.push(local);
                    try {
                        renderElements(fe.body, scopes, sb);
        ...
        if (el instanceof WhileNode) {
            ...
            while (toBoolean(evalExpression(wn.condition, scopes))) {
                try {
                    ...
                    if ("break".equals(cf.tag)) break;
                    if ("continue".equals(cf.tag)) continue;
                }
            }
            return;
        }
        ...
        if (el instanceof IncludeNode) {
            ...
            if (baseDir != null) {
                try {
                    Path p = baseDir.resolve(inc.path).normalize();
                    if (Files.exists(p)) {
                        String txt = Files.readString(p);
                        // TODO: handle further render
                        sb.append(txt);
                    }
                } catch (Exception ex) {
                    sb.append("<!-- include error: ").append(ex.getMessage()).append(" -->");
                }
            }
            return;
        }
    ...
    private Object evalExpression(ExpressionNode expr, Deque<Map<String,Object>> scopes) {
        ...
        if (expr instanceof FunctionCallNode) {
            ...
            for (ExpressionNode e : f.args) args.add(evalExpression(e, scopes));
        ...
        if (expr instanceof UnaryOpNode) {
            ...
            String op = u.op;
            if ("!".equals(op)) return !toBoolean(v);
            if ("-".equals(op)) {
                Number n = toNumber(v);
                if (n == null) return 0;
        ...
        if (expr instanceof TernaryNode) {
            ...
            if (toBoolean(c)) return evalExpression(t.thenExpr, scopes);
    ...
    private void setIdentifier(String name, Object value, Deque<Map<String,Object>> scopes) {
        for (Map<String,Object> s : scopes) {
            if (s.containsKey(name)) { s.put(name, value); return; }
        }
    ...
    private Object callFunction(Object callee, List<Object> args) {
        ...
        if (callee instanceof String) {
            ...
            if ("len".equals(name) && args.size() == 1) {
                ...
                if (a instanceof Collection) return ((Collection<?>)a).size();
                if (a instanceof Map) return ((Map<?,?>)a).size();
                if (a != null && a.getClass().isArray()) return java.lang.reflect.Array.getLength(a);
                return 0;
            }
        }
    ...
    private Number toNumber(Object o) {
        ...
        try {
            if (o.toString().contains(".")) return Double.parseDouble(o.toString());
    ...
    private Object applyBinary(Object L, String op, Object R) {
        ...
        // comparisons
        if (">".equals(op)) return toNumber(L).doubleValue() > toNumber(R).doubleValue();
        if ("<".equals(op)) return toNumber(L).doubleValue() < toNumber(R).doubleValue();
        if (">=".equals(op)) return toNumber(L).doubleValue() >= toNumber(R).doubleValue();
        if ("<=".equals(op)) return toNumber(L).doubleValue() <= toNumber(R).doubleValue();
        ...
        // logical
        if ("&&".equals(op)) return toBoolean(L) && toBoolean(R);
        if ("||".equals(op)) return toBoolean(L) || toBoolean(R);
